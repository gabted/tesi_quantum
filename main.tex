...
Linear qCCS (from now on, lqCCS) allows only for \textit{asynchronous communication} \cite{boudolAsynchronyPicalculus1992}, meaning that after an output action there cannot be other action. In a asyncrhonous calculus a process like $c!v_1.c!v_2.P$ is not a valid process, and is often written as $P \parallel c!v_1\parallel c!v_2$. This means that the output actions are "non-blocking", the process $P$ can perform other steps of the computation without having to wait the "reception" of the sent value. An other consequence is that $P$ has no control on the order in which $v_1$ and $v_2$ are received. \note{the advantages of asynchronous calculus are ...}

\note{ha tutte le feature quantum: superoperatori e misure, e tutte le feature classiche, somme e paralleli. Ha anche un costrutto condizionale al posto della guarda di qCCS. NOn ha processi ricorsivi.}
