...
We define as a barb, the atomic observable property, only the capability of performing an output on a channel. We do not consider input action observables, as it is the norm \note{[cite amadio sangiorgi bisimulation for asynch pi]} for asynchronous calculi. Since the external observer is an asynchronous, non blocking context, it can not observe the process $P$ to discorver if it has a $c?x$ transition available: the context can only perform a terminal action $c!v$, but it will not know if the output action is ever received.
...
simPS is designed to be equivalent to open bisimulation for qCCS, except for a few intended modification:\begin{itemize}
...
\begin{theorem}[$\sim$ is closed for additional discarded qubits]\label{bisim_closed_by_discard}
...
Since $tr_q(\sop_{H, q}(\sigma)) = tr_q(\sop_{Z, q}(\sigma)) = tr_q(\sigma)$ for lemma \note{LEMMA}, we have that $\confw{\sop_{H, q}(\sigma), R \parallel disc(q)} \sim_{PS} \confw{\sop_{X, q}(\sigma), R \parallel disc(q)}$ for lemma \ref{trace_and_sop}, and so $\confw{\sop_{H, q}(\sigma), R \parallel disc(q)} \rel \confw{\sop_{X, q}(\sigma), R \parallel disc(q)}$ by definition of $\rel$.
